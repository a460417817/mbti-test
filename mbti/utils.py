...
def get_questions():
    ...
    questions = []
    with open(question_txt, encoding='utf-8') as txt:
        for line in txt:
            question, choice_a, choice_b = line.strip().split()
            questions.append({
                'question': question,
...
def get_types_desc():
    ...
    types_desc = {}
    with open(types_txt, encoding='utf-8') as types:
        for line in types:
            typ3, desc = line.strip().split()
